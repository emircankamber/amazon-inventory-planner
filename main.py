  ...
  <div class="max-w-6xl mx-auto px-6 py-10">
    <div class="flex items-start justify-between gap-6 flex-wrap">
      <div>
        ...
        <p class="text-slate-300 mt-2">Tek kullanıcı (login yok) — Silme + Excel Export var.</p>
      </div>
...
def fetch_month_units(cur, sku: str, y: int, m: int) -> Optional[int]:
    row = cur.execute("SELECT units_sold FROM monthly_sales WHERE sku=? AND year=? AND month=?", (sku, y, m)).fetchone()
    return None if row is None else int(row["units_sold"])


def delete_product_and_sales(cur, sku: str):
    cur.execute("DELETE FROM monthly_sales WHERE sku=?", (sku,))
    cur.execute("DELETE FROM products WHERE sku=?", (sku,))


def delete_single_month_sale(cur, sku: str, year: int, month: int):
    cur.execute("DELETE FROM monthly_sales WHERE sku=? AND year=? AND month=?", (sku, year, month))
...
def upsert(
    ...
    conn = get_conn()
    cur = conn.cursor()
    upsert_product(cur, sku, name.strip(), int(lead_time_days), float(z_value), int(fba_stock), int(inbound_stock))
...
def delete_product(sku: str):
    conn = get_conn()
    cur = conn.cursor()
    delete_product_and_sales(cur, sku)
    conn.commit()
    conn.close()
...
def delete_sale(sku: str = Form(...), year: int = Form(...), month: int = Form(...)):
    conn = get_conn()
    cur = conn.cursor()
    delete_single_month_sale(cur, sku, int(year), int(month))
    conn.commit()
    conn.close()
    return RedirectResponse(url=f"/product/{sku}", status_code=303)
  ...
  <div class="lg:col-span-2 space-y-6">
    ...
    <div class="rounded-3xl border border-slate-700/60 bg-slate-900/40 p-6">
      ...
      <form class="mt-4 grid grid-cols-1 md:grid-cols-4 gap-3" method="post" action="/delete/sale"
        ...
        <input class="rounded-2xl bg-slate-900/60 border border-slate-700 px-4 py-3" name="year" type="number" placeholder="Yıl" required>
        <input class="rounded-2xl bg-slate-900/60 border border-slate-700 px-4 py-3" name="month" type="number" min="1" max="12" placeholder="Ay" required>
...
<script>
  ...
  new Chart(ctx, {{
    type: 'line',
    data: {{ labels, datasets: [{{ label: 'Aylık Satış', data, tension: 0.3 }}] }},
